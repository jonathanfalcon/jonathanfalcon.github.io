...
import { getHashDataUrl } from '@/utils/hash/hash'
import HashPicture from '@/utils/hash/HashPicture.astro'
import ColorMode from '@/components/Footer/ColorMode.astro'
import { siteConfig } from "@/config/siteConfig"

import thumbHashExample from './images/personal-site.png'
...
<HashPicture src='./images/astro.png' alt='Astro banner saying build the web you want'/>
...
<HashPicture src='./images/tailwind.jpg' alt='Tailwind CSS banner'/>
...
<div className='not-prose flex flex-row items-center gap-x-4'>
    <figure
        className='flex aspect-video w-1/2 items-center justify-center rounded-lg bg-emerald-900 text-center font-heading text-3xl font-black text-emerald-50 md:text-5xl'>
        Emerald
    </figure>
    <figure
        className='flex aspect-video w-1/2 items-center justify-center rounded-lg bg-slate-200 text-center font-heading text-3xl font-black dark:bg-slate-800 md:text-5xl'>
        Slate
    </figure>
</div>
...
<div className='not-prose flex flex-row items-center gap-x-4'>
    ...
    <figure
        className='flex aspect-video w-1/2 items-center justify-center rounded-lg bg-slate-200 text-center font-body text-3xl font-medium dark:bg-slate-800 md:text-5xl'>
        Inter
    </figure>
</div>
...
<figure
    className='not-prose mx-auto flex aspect-video h-full w-2/3 max-w-[376px] items-center justify-center rounded-lg bg-slate-200 p-10 dark:bg-slate-800'>
    <Icon name={siteConfig.icon} class='aspect-square size-1/2'/>
</figure>
...
<div className='not-prose grid w-full gap-4 md:grid-cols-2 grid-cols-1'>
    ...
    <div
        ...
        style={`background: center / cover url(${await getHashDataUrl({src: thumbHashExample.src})});`}
        aria-label='ThumbHash representation of the prior image.'
    ></div>
</div>
...
<img
    ...
    style="aspect-ratio: 16/9;
           background: center / cover
                       url(data:image/webp;base64,thumbhash-image-url);"
/>
```
...
<HashPicture
    src='./images/thumbhash-header-example.png'
    alt='Screenshot of thumbhash header example'
/>
...
<div className='mx-auto flex aspect-video size-full max-w-[376px] items-center justify-center rounded-lg bg-slate-800 text-base'>
    <ColorMode isExample />
</div>
