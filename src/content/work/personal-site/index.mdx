...
BlurHashes offer a compact representation of images, enabling the display of a blurred version while the full image is loading. This not only adds an aesthetic flair by providing users with a visually pleasing preview but also reduces perceived latency.
...
<div className='not-prose grid w-full gap-4 md:grid-cols-2 grid-cols-1'>
    ...
    <div
        ...
        style={`background: center / cover url(${await getThumbHashDataUrl(blurHashExample.src)});`}
...
<ThumbHashPicture
    src='./images/blurhash-header-example.png'
