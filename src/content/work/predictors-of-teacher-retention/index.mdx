...
Thus, we create the `where_binary()` function. It accepts a column and a condition. It first checks for `NaN` values and assigns `np.NaN` when true. When false, the condition is evaluated in a nested condition, assigning `1` if true and `0` if false.
